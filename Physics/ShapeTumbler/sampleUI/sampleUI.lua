...
function M:newUI( options )
	...
	local barContainer = display.newContainer( display.contentWidth, 36 ) ; frontGroup:insert( barContainer ) ; barContainer.anchorY = 0
	...
	local background = display.newImageRect( "sampleUI/back-"..themeName..".png", 360, 640 )
	background.x, background.y = display.contentCenterX, display.contentCenterY

	local topBarBack = display.newRect( frontGroup, 0, 0, display.contentWidth, 37 )
	topBarBack:setFillColor( 0,0,0,0.2 ) ; topBarBack.anchorY = 0 ; topBarBack:toBack()
	local topBarOver = display.newRect( barContainer, 0, 0, display.contentWidth, 36 )
	...
	if ( system.orientation == "landscapeLeft" or system.orientation == "landscapeRight" ) then
		...
		backgroundScaler = math.max( display.contentHeight / background.width, display.contentWidth / background.height )
	elseif ( system.orientation == "portrait" or system.orientation == "portraitUpsideDown" ) then
		...
		backgroundScaler = math.max( display.contentHeight / background.height, display.contentWidth / background.width )
	end

	print( "Scaler: ", backgroundScaler)
	background:scale( backgroundScaler, backgroundScaler )
